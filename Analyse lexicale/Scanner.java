...
public class Scanner {
   ...
   public UniteLexicale lexemeSuivant() {
		...
		if(caractereCourant==')')
			return new UniteLexicale(Categorie.PARENFERM, ")");

	
         if(caractereCourant==':')
			return new UniteLexicale(Categorie.DPOINT, ":");


		

        if(caractereCourant=='[')
			return new UniteLexicale(Categorie.CROCHOUV, "[");

		if(caractereCourant==']')
			return new UniteLexicale(Categorie.CROCHFERM, "]");
		...
		while(true) {
			switch(etat) {
				...
				case 1 : caractereSuivant();
						 ...
						 break;
				case 2 : reculer();
                            //La fonction VerifMotCle permet t=de vérifier s'il s'agit d'un mot clé ou un ID
                            if(VerifMotcle(sb.toString())){
                                return new UniteLexicale(sb.toString(), sb.toString()) ;} // <si,si>
                            else
